...
import net.hashsploit.ps2.uya.medius.utils.TextbookRSA;
...
class TestRSA {
	...
	public static void handle92() {
		...
		BigInteger q1 = q.subtract(s);
		BigInteger p1 = p.subtract(s);
		BigInteger pq = q1.multiply(p1);
		BigInteger d = e.modInverse(pq);

		TextbookRSA rsa = new TextbookRSA(
				// mod
				// new
				// BigInteger("C4F75716EC835D2325689F91FF85ED9BFC3211DB9C164F41852E264E569D2802008054A0EF459E7E3EABB87FAE576E735434D1D124B30B11BD6DE09814860155",
				// 16),
				n,
				...
				// private exp
				// new
				// BigInteger("4854567300243763614870687120476899445974505675147434999327174747312047455575182761195687859800492317495944895566174677168271650454805328075020357360662513",
				// 10)
				d);

		System.out.println("------------- 92 MESSAGE --------------");
		BigInteger signed = m_92.modPow(e, n); // rsa.Encrypt(m_92);
		byte[] signedBytes = signed.toByteArray();
		
		System.out.println("Signed: " + Utils.baToString(signedBytes));

		BigInteger unsigned = signed.modPow(d, n);
		byte[] unsignedBytes = unsigned.toByteArray();
		System.out.println("Unsigned: " + Utils.baToString(unsignedBytes));

		System.out.println("DECRYPTION " + (unsigned.compareTo(m_92) == 0 ? "WORKED!!!!!!!!!!!!!!!!!!!!" : "FAILED"));
	}

	public static void handle93() {
		BigInteger c_93 = new BigInteger(
				"AF9CBD1868A8C3CA4E547D4C0177FA92A7E475A83DB2F96CE1665BDB8048EE505968759EB8624345E1A56805E891344A2ADD26B2E32CA6DCD96E50414C8B7A1A",
				16);
		BigInteger m_93 = new BigInteger(
				"8694D579A31ADE7F3B37E08C520051BFC74D20F0D5F5A65C8D4488D4173538DBFA2ED807E48B9C326A884689DACE2347F0843DD96DF20D5E0BEF70D15C7E9360",
				16);

		BigInteger c_94 = new BigInteger(
				"403B472000686E07997AAA5260FCDF6B0B2860872A232FABEE396B6A664263813ACCBD1880A72BB78AD5697F5D114C9ACB4A971858D10A1F7D32DD27F3D06C73",
				16);
		...
		TextbookRSA rsa = new TextbookRSA(
				...
				// private exp
				new BigInteger(
						"85A8EC2D3002C63B9E4AF4C014248F3224B47C14E1F45A5E4980BB1BB370F26DD8B80978FC2DCEC8B28563F1659A00B65C843D20732D3773E6AA95C37F9D5511",
						16));
		...
		System.out.println("DECRYPTION " + (unsigned.compareTo(m_93) == 0 ? "WORKED!!!!!!!!!!!!!!!!!!!!" : "FAILED"));
	}
